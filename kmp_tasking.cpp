...
typedef enum
{
	Kmp_alloc_task_event,
	Kmp_push_task_event,
	Kmp_steal_task_event,
	Kmp_execute_task_event,
	Kmp_num_events
}KMP_Events;
#define NUM_EVENTS	3
long_long values[Kmp_num_events][NUM_EVENTS];
int Events[NUM_EVENTS]={PAPI_TOT_INS,PAPI_TOT_CYC, PAPI_L1_DCM};
/* ------------------------------------------------------------------------ */
...
__kmp_push_task(kmp_int32 gtid, kmp_task_t * task )
{
    ...
    KA_TRACE(20, ("__kmp_push_task: T#%d trying to push task %p My_GTID = %d.\n", gtid, taskdata, taskdata->td_task_id ) );
    ...
    if ( taskdata->td_flags.tiedness == TASK_UNTIED ) {
        ...
        KA_TRACE(20, ( "__kmp_push_task: T#%d untied_count (%d) incremented for task %p My_GTID = %d\n",
                       gtid, counter, taskdata, taskdata->td_task_id ) );
    }
    ...
    PAPI_start_counters((int*)Events,NUM_EVENTS);	//Shayan PAPI
#if OMP_45_ENABLED
    // Need to recheck as we can get a proxy task from a thread outside of OpenMP
    if ( TCR_4(thread_data -> td.td_deque_ntasks) >= TASK_DEQUE_SIZE(thread_data->td) )
    {
        __kmp_release_bootstrap_lock( & thread_data -> td.td_deque_lock );
        KA_TRACE(20, ( "__kmp_push_task: T#%d deque is full on 2nd check; returning TASK_NOT_PUSHED for task %p My_GTID = %d\n",
                       gtid, taskdata, taskdata->td_task_id ) );
        return TASK_NOT_PUSHED;
    }
#else
    ...
    PAPI_stop_counters((values[Kmp_push_task_event]), NUM_EVENTS);	//Shayan PAPI

    KA_TRACE(20, ("__kmp_push_task: T#%d returning TASK_SUCCESSFULLY_PUSHED: "
          "task=%p ntasks=%d head=%u tail=%u My_GTID = %d PAPI Information: Instructions: %llu Cycles: %llu L1 Misses: %llu\n",
          gtid, taskdata, thread_data->td.td_deque_ntasks,
          thread_data->td.td_deque_head, thread_data->td.td_deque_tail, taskdata->td_task_id, values[Kmp_push_task_event][0], values[Kmp_push_task_event][1], values[Kmp_push_task_event][2]) );
...
__kmp_task_start( kmp_int32 gtid, kmp_task_t * task, kmp_taskdata_t * current_task )
{
    ...
    KA_TRACE(10, ("__kmp_task_start(enter): T#%d starting task %p: current_task=%p (Current Tasks My_GTID = %d)(New Tasks My_GTID = %d)\n",
                  gtid, taskdata, current_task, current_task->td_task_id, taskdata->td_task_id) );
    ...
    KA_TRACE(10, ("__kmp_task_start(exit): T#%d task=%p My_GTID = %d\n",
                  gtid, taskdata, taskdata->td_task_id ) );
...
//TODO SHAYAN
/*kmp_taskdata_t *taskdata; see if you can use this line and its data structure to uniquely identify
 * a task by its global ID maybe??*/
//---------------------------------------------------------------------------------
                  ...
                  kmp_routine_entry_t task_entry )
{
    ...
    taskdata->td_task_id      = KMP_GEN_TASK_ID();			//TODO SHAYAN use this as global task ID
...
#endif
    //KA_TRACE(20, ("__kmp_task_alloc(exit): Shayan: Global Task ID For Created Task:%d\n",taskdata->td_task_id));
    ...
    PAPI_stop_counters((values[Kmp_alloc_task_event]), NUM_EVENTS);	//Shayan PAPI
    KA_TRACE(20, ("__kmp_task_alloc(exit): T#%d created task %p parent=%p New Tasks My_GTID = %d, Parents Task My_GTID = %d PAPI Information: Instructions: %llu Cycles: %llu L1 Misses: %llu\n",
    		gtid, taskdata, taskdata->td_parent, taskdata->td_task_id, taskdata->td_parent->td_task_id,values[Kmp_alloc_task_event][0], values[Kmp_alloc_task_event][1], values[Kmp_alloc_task_event][2] ));
...
//TODO SHAYAN
/*This is where the task is created and pushed to queue, if its
 * not pushed to queue then it is executed immediately*/
//---------------------------------------------------------------------
...
//TODO SHAYAN
/*
 * this is where the task gets scheduled*/
kmp_int32
...
#endif
    	//TODO SHAYAN
    	//invoke_task means execute task
    {                                                           // Execute this task immediately
        kmp_taskdata_t * current_task = __kmp_threads[ gtid ] -> th.th_current_task;
...
__kmpc_omp_task( ident_t *loc_ref, kmp_int32 gtid, kmp_task_t * new_task)
{
    ...
    kmp_taskdata_t * new_taskdata = KMP_TASK_TO_TASKDATA(new_task);
#endif
    KA_TRACE(10, ("__kmpc_omp_task(enter): T#%d loc=%p task=%p My_GTID = %d\n",
                  gtid, loc_ref, new_taskdata, new_taskdata->td_task_id ) );

    res =  __kmp_omp_task(gtid,new_task,true);

    KA_TRACE(10, ("__kmpc_omp_task(exit): T#%d returning TASK_CURRENT_NOT_QUEUED: loc=%p task=%p, My_GTID = %d\n",
                  gtid, loc_ref, new_taskdata, new_taskdata->td_task_id ) );
    return res;
}
                  ...
                  kmp_int32 is_constrained )
{
    ...
    PAPI_stop_counters((values[Kmp_steal_task_event]), NUM_EVENTS);	//Shayan PAPI
    __kmp_release_bootstrap_lock( & victim_td -> td.td_deque_lock );

    KMP_COUNT_BLOCK(TASK_stolen);
    KA_TRACE(10, ("__kmp_steal_task(exit #3): T#%d stole task %p (My_GTID = %d) from T#%d: task_team=%p "
                  "ntasks=%d head=%u tail=%u PAPI Information: Instructions: %llu Cycles: %llu L1 Misses: %llu\n",
                  gtid, taskdata, taskdata->td_task_id, __kmp_gtid_from_thread( victim ), task_team,
                  victim_td->td.td_deque_ntasks, victim_td->td.td_deque_head,
                  victim_td->td.td_deque_tail, values[Kmp_steal_task_event][0] ,values[Kmp_steal_task_event][1], values[Kmp_steal_task_event][2]) );
...
//TODO SHAYAN
/*to execute the task
 * This will check own queue for a task use it
 * if not then try and steal a task
 * break out of the loop if you dont have a task AND no other task to steal*/
//-----------------------------------------------------------------------------
